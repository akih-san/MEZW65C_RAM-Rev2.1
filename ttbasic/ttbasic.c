...
static void basic(void); /* prototype */
...
#define SIZE_LIST 31160 /* List buffer size 30720bytes */
...
#define STR_EDITION "MEZW65C_RAM"
...
static short getrnd(short value){
	return(rand() % value) + 1;
}

/* Prototypes (necessity minimum) */
static short iexp(void);

/* Keyword table */
static const char* kwtbl[] = {
	"GOTO", "GOSUB", "RETURN",
	"FOR", "TO", "STEP", "NEXT",
	"IF", "REM", "STOP",
	"INPUT", "PRINT", "LET",
...
/* no space after */
static const unsigned char i_nsa[] = {
...
static const unsigned char i_nsb[] = {
	I_MINUS, I_PLUS, I_MUL, I_DIV, I_OPEN, I_CLOSE,
	I_GTE, I_SHARP, I_GT, I_EQ, I_LTE, I_LT,
...
static char sstyle(unsigned char code, const unsigned char *table, unsigned char count) {
	while(count--) {
...
/* RAM mapping */
static char lbuf[SIZE_LINE]; /* Command line buffer */
static unsigned char ibuf[SIZE_IBUF]; /* i-code conversion buffer */
static short var[26]; /* Variable area */
static short arr[SIZE_ARRY]; /* Array area */
static unsigned char listbuf[SIZE_LIST]; /* List area */
static unsigned char* clp; /* Pointer current line */
static unsigned char* cip; /* Pointer current Intermediate code */
static unsigned char* gstk[SIZE_GSTK]; /* GOSUB stack */
static unsigned char gstki; /* GOSUB stack index */
static unsigned char* lstk[SIZE_LSTK]; /* FOR stack */
static unsigned char lstki; /* FOR stack index */

/* Standard C libraly (about) same functions */
static char c_toupper(char c) {return(c <= 'z' && c >= 'a' ? c - 32 : c);}
static char c_isprint(char c) {return(c >= 32  && c <= 126);}
static char c_isspace(char c) {return(c == ' ' || (c <= 13 && c >= 9));}
static char c_isdigit(char c) {return(c <= '9' && c >= '0');}
static char c_isalpha(char c) {return ((c <= 'z' && c >= 'a') || (c <= 'Z' && c >= 'A'));}
static void c_puts(const char *s) {while(*s) c_putch(*s++);}
...
static void putnum(short value, short d){
	unsigned char i;
	unsigned char sign;
...
static short getlineno(unsigned char *lp) {
	if(*lp == 0) /*end of list */
...
static unsigned char* getlp(short lineno) {
	unsigned char *lp;
...
static void putlist(unsigned char* ip) {
	unsigned char i;
